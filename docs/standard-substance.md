# Standard Substance | Shanghai Kepon Biotech / KPBioIE
[Certified reference materials and standard solutions](https://global.kpbioie.com/product-category/standard_substance/) for instrument calibration, pharmaceutical testing, environmental monitoring and laboratory quality control.

## Main Product Range
- Titration Solutions
- Buffer Solutions
- Instrument Calibration Standards
- Pharmaceutical Testing Standards
- Biochemical Assay Standards
- Element Standards and Inorganic Solutions
- Organic Reference Materials
- Physical Property Standards
- VOCs (Volatile Organic Compounds) Standards
- Environmental Monitoring Series
  - Water Quality Analysis
  - Mixed Anion Standards
  - Mixed Cation Standards
- Soil & Agriculture Testing Standards
- Food & Nutrition Testing Standards
- Custom R&D reference materials

## Custom Service
Custom concentration, customized purity specification, COA certificate provided and full export compliance support.

---
## Contact & Inquiry
- Official Website: https://global.kpbioie.com/
- Contact Page: https://global.kpbioie.com/contact/
- Business Email: info@kpbioie.com
- Phone: +86 15000556234
- WhatsApp: +86 15000556234

## Ordering Information
Please indicate purity, concentration, execution standard, and testing purpose. Hard-to-find custom standards are supported via email inquiry.
